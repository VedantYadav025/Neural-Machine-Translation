...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[
        ...
        source_length, batch_size = source_padded.shape

        X = self.model_embeddings.source(source_padded)  # Embedding tensor of shape ()
        # print(X.shape)
        _ , __, e = X.shape
        X_for_conv = torch.permute(X, (1 , 2, 0))
        conv_X = self.post_embed_cnn(X_for_conv)
        X = torch.permute(conv_X, (2, 0, 1))
        X = pack_padded_sequence(X, source_lengths)  # can be passed into the encoder (now, LSTM will know which tokens are result of padding, and hence will ignore them)
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        enc_hidden_proj = self.att_projection(enc_hiddens)   # this will take dot product with decoder hidden state to compute attention scores
        Y = self.model_embeddings.target(target_padded)  # tensor of shape (tgt_len, b, e) --> no clue what this does

        for y_t in torch.split(Y, 1, dim=0):   # y.shape = [1, b, e]
            Y_t = torch.squeeze(y_t, dim=0)    # squeezing along the time dimension
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        """ Compute one forward step of the LSTM decoder, including the -> Tuple[Tuple, torch.Tensor, torch.Tensor]attention computation.
        ...
        # print(f"Batch size is {Ybar_t.size(0)}")

        dec_state = self.decoder(Ybar_t, dec_state) # this is the first return 
        dec_hidden, dec_cell = dec_state
        e_t = torch.bmm(enc_hiddens_proj, dec_hidden.unsqueeze(-1))  # shape of e_t is (b, src_len, 1)
        ...
        alpha_t = F.softmax(e_t, dim=1)  # dim is (b, src_len)
        a_t = torch.bmm(torch.unsqueeze(alpha_t, dim=1), enc_hiddens)  # a_t.shape = [b, 1, 2h]
        a_t = a_t.squeeze(dim=1)  # a_t.shape = [b, 2h]
